//Scripts para automatizar login en https://opensource-demo.orangehrmlive.com/
...
import dashBoard from '../pages/inventoryPage'
...
test.only('El usuario debe iniciar sesión correctamente', async t => {
    ...
    await t.expect(dashBoard.title.exists).ok()
} )

// TC-02 El usuario ingresó valores incorrectos en los campos user y password
test.only('Usuario y password incorrectos', async t => {
...
test.only('Campo de usuario vacío', async t => {
